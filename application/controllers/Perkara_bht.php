...
class Perkara_bht extends REST_Controller {
    ...
    function index_get() {
       ...
       if($q3 == "gugatan"){
            ...
            $data_perkara = $this->db->query("SELECT B.nomor_perkara, 
            DATE_FORMAT(A.tanggal_putusan,'%d/%m/%Y') AS tanggal_putusan,
            CONCAT(SUBSTRING_INDEX(C.majelis_hakim_nama, '<', 1) ,' / ' , REPLACE(C.panitera_pengganti_text, 'Panitera Pengganti: ', ''))  AS majelis, B.jenis_perkara_nama,
            ...
            WHERE tanggal_bht = '$q' AND C.panitera_pengganti_text LIKE '%$q2%'
            ...
            foreach ($data_perkara as $key => $value) {
                $d[] = [$no,$value->nomor_perkara,$value->majelis,$value->jenis_perkara_nama,$value->tanggal_putusan, $value->tanggal_bht];
                $no++;
            }
            ...
            CONCAT(SUBSTRING_INDEX(C.majelis_hakim_nama, '<', 1) ,' / ' , REPLACE(C.panitera_pengganti_text, 'Panitera Pengganti: ', ''))  AS majelis, B.jenis_perkara_nama,
            DATE_FORMAT(A.tanggal_minutasi,'%d/%m/%Y') AS tanggal_minutasi,
            DATE_FORMAT(DATE_ADD(A.tanggal_putusan , INTERVAL 14 DAY),'%d/%m/%Y') AS tanggal_bht
            FROM perkara_putusan AS A
            LEFT JOIN perkara AS B ON A.`perkara_id` = B.`perkara_id`
            LEFT JOIN perkara_penetapan AS C ON A.`perkara_id` = C.`perkara_id`
            WHERE tanggal_putusan < CURDATE() - INTERVAL 14 DAY
            AND tanggal_putusan = DATE_ADD('$q' , INTERVAL -14 DAY)
            AND C.panitera_pengganti_text LIKE '%$q2%'
