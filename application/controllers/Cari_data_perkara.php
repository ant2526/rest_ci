...
class Cari_data_perkara extends REST_Controller {
    ...
    function index_get() {
        ...
        $data_perkara = array();
        if ($q != '') {
        $data_perkara = $this->db->query("SELECT A.perkara_id, A.nomor_perkara, B.nama AS nama_p, B.alamat AS alamat_p, C.nomor_indentitas AS nik_p, C.telepon AS nomor_hp_p, D.nama AS nama_t, D.alamat AS alamat_t, E.nomor_indentitas AS nik_t, E.telepon AS nomor_hp_t
FROM perkara AS A 
