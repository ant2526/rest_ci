...
class Ecourt extends REST_Controller {
   ...
   function index_get() {
    ...
    $data_perkara = array();
    if($mp == 0){
        $data_perkara = $this->db->query("SELECT efiling_id, perkara.tanggal_pendaftaran, perkara.nomor_perkara, perkara_putusan.tanggal_putusan, proses_terakhir_text, hakim_kode, hakim_nama FROM perkara_efiling LEFT JOIN perkara ON perkara_efiling.nomor_perkara = perkara.nomor_perkara LEFT JOIN perkara_putusan ON perkara.`perkara_id` = perkara_putusan.`perkara_id` LEFT JOIN perkara_hakim_pn ON perkara.perkara_id = perkara_hakim_pn.perkara_id WHERE tahapan_terakhir_id = 12 AND jabatan_hakim_id = 1 AND perkara.tanggal_pendaftaran BETWEEN '$q' AND '$q2' GROUP BY efiling_id");
        ...
        $data_perkara = $this->db->query("SELECT efiling_id, perkara.tanggal_pendaftaran, perkara.nomor_perkara, perkara_putusan.tanggal_putusan, proses_terakhir_text, hakim_kode, hakim_nama FROM perkara_efiling LEFT JOIN perkara ON perkara_efiling.nomor_perkara = perkara.nomor_perkara LEFT JOIN perkara_putusan ON perkara.`perkara_id` = perkara_putusan.`perkara_id` LEFT JOIN perkara_hakim_pn ON perkara.perkara_id = perkara_hakim_pn.perkara_id WHERE tahapan_terakhir_id >= 15 AND jabatan_hakim_id = 2 AND perkara_putusan.`tanggal_putusan` BETWEEN '$q' AND '$q2' GROUP BY efiling_id");
         $data_perkara = $data_perkara->result();
    } 
        ...
        foreach ($data_perkara as $key => $value) {
            $d[] = [$no,$value->nomor_perkara,$value->tanggal_pendaftaran,$value->tanggal_putusan,$value->proses_terakhir_text];
            $no++;
        }
